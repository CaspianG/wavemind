...
| Memory OS runtime soak | Real Redis concurrency and retry proof for atomic lock ownership, lease heartbeat, one completed mutation per run id, duplicate retry suppression, and failed-job retry. | `benchmarks/memory_os_runtime_soak_results.json`, `benchmarks/MEMORY_OS_RUNTIME_SOAK.md`, `benchmarks/memory_os_runtime_soak.py` | Local Docker Redis passes 20 rounds with 4 contenders, 20 completed runs, 60 safe lock skips, zero retry mutation delta, 12 lease refreshes, and zero errors. A remote production-like Redis run is still required for admission. |
| Memory OS remote worker soak | Production admission requires direct concurrency across at least two authenticated HTTPS workers and their shared TLS Redis. The runner checks matching versions, safe plans, cross-worker single-flight, completed-job retry suppression, lease safety, and cleanup. | `benchmarks/memory_os_remote_worker_soak.py`, `benchmarks/memory_os_remote_soak_handoff_results.json`, `benchmarks/MEMORY_OS_REMOTE_SOAK_HANDOFF.md`, `.github/workflows/memory-os-remote-soak.yml` | Runner-ready. Configure the `memory-os-production-evidence` GitHub Environment with worker URLs, Redis URL, and an admin API key, then dispatch the workflow. A legacy artifact that only says `remote_redis` cannot unlock production. |
| Memory OS quality gate | A single regression gate joins direct Memory OS agent-coherence evidence with LoCoMo retrieval, LongMemEval retrieval, and LongMemEval answer-generation evidence. All `7/7` checks pass: agent task success is `0.9167`, stale error is `0`, context saved is `0.9309`, LoCoMo recall lift is `0.1380`, LongMemEval recall lift is `0.2625`, and answer F1 lift is `0.1632`. | `benchmarks/memory_os_quality_results.json`, `benchmarks/MEMORY_OS_QUALITY.md`, `benchmarks/memory_os_quality_gate.py` | Memory OS non-regression is measured directly on agent coherence. LoCoMo and LongMemEval prove the underlying dynamic retrieval and answer context; they do not claim unattended workers ran inside those public datasets. |
| Memory OS policy bundle | Operator-facing runtime policy manifest generated from canary, policy-evolution, and admission artifacts. It emits enabled task ids, required Redis/lock env, staged rollout gates, emergency stop, rollback policy, observability metrics, and Kubernetes/CronJob patch data. | `benchmarks/memory_os_policy_bundle_results.json`, `benchmarks/MEMORY_OS_POLICY_BUNDLE.md`, `wavemind memory-os-policy-bundle --write-artifacts` | Current status is `staging_ready`: `6/7` checks pass; production stays locked only on the remote runtime soak. Automatic promotion remains disabled. |
| Memory OS admission | Deployment-facing gate for adaptive workers. It checks scheduler safety, hot-query audit signal, Redis cache wiring, atomic distributed lease wiring, singleton/idempotent mutations, policy coverage, runtime soak evidence, and architecture boundaries before Memory OS workers become production automation. | `benchmarks/memory_os_admission_results.json`, `benchmarks/MEMORY_OS_ADMISSION.md`, `wavemind memory-os-admission --target-memories 50000 --namespace-count 64 --deployment production --runtime-evidence <downloaded-remote-worker-soak.json> --fail-on-blocked` | Current 50k production profile passes `11/12` requirements. Its only blocker is `runtime-soak`: production now requires the combined secure multi-worker artifact generated by `.github/workflows/memory-os-remote-soak.yml`. |
