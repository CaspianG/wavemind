...
| Structured payloads | image/audio/video/3D/table/event/graph retrieval through the standard memory API, precision@1 `1.000`, p99 `0.95 ms`; cross-modal target-modality retrieval over persisted payload vectors, precision@1 `1.000`, vector persistence `1.000`, provenance rate `1.000`, embedding dim `64`, p99 `2.43 ms`; strict external/precomputed vectors for image/audio/video/3D, precision@1 `1.000`, vector persistence `1.000`, p99 `1.12 ms`; temporal event retrieval covers actor filters, interval overlap, around-time reranking, recency reranking, persistence, and provenance with precision@1 `1.000`, p99 `1.24 ms`. |
