...
scale claims. The persisted 50M FAISS profile now passes this gate.
...
artifact passes. Persisted 50M FAISS is admitted; the unfinished 10M service and
100M sharded profiles remain blocked. `--allow-plan-only` reports the next run
...
| Strict production evidence | The gate now passes `1/8` requirements: persisted 50M FAISS is proven at recall@10 `0.9705` and p99 `73.11 ms`; seven remote, managed-serverless, 10M-service, and 100M requirements remain. | `benchmarks/production_evidence_results.json`, `benchmarks/PRODUCTION_EVIDENCE.md`, `benchmarks/production_evidence_gate.py`, `wavemind production-evidence --strict` | Overall status remains action-required until the seven real remote/service artifacts are committed. |
...
| Kubernetes physical worker failure | Four real WaveMind pod-DNS endpoints across three kind worker zones retain `1.00` recall while one worker container is physically paused, then recover with no failed nodes. | `benchmarks/kubernetes_cluster_network_smoke_results.json`, `benchmarks/kubernetes_cluster_network_smoke.py`, `.github/workflows/kubernetes-operator-smoke.yml` | This proves the non-loopback service-network and quorum behavior in ephemeral CI. It does not replace a remote node/zone/region drill or unlock strict cluster admission. |
...
| Agent coherence and token savings | On a 500-memory long user-history task simulation, WaveMind reaches `0.92` task success, `0.00` stale error rate, `0.93` context saved, `9` coherent turns, and `2.65 ms` average query latency. WaveMind + Memory OS keeps `0.92` task success while proving `1` hot query, `1` prewarmed query, `4` predictive-prefetch warmed queries, `5` priority predictions, and cache hit rate `0.24`. Static vector reaches `0.33` task success and `0.73` stale error rate; Chroma static reaches `0.33` task success and `0.45` stale error rate. | This measures agent behavior rather than nearest-neighbor retrieval: fewer stale facts, longer coherent task runs, compact context, and observable Memory OS learning signals. |
...
| Streaming production load | `benchmarks/production_streaming_load_benchmark.py` generates and inserts vectors in batches, stores only query source vectors outside the index, and measures target-recall, p99, SLO, and cost. The checked-in 10M compressed FAISS IVF-PQ run reaches recall@10 `0.990`, p99 `60.13 ms`; the real 50M persisted FAISS run reaches recall@10 `0.9705`, p99 `73.11 ms` over `2,000` queries after an adaptive `nprobe` sweep; tuned 1M Qdrant reaches recall@10 `1.000`, p99 `26.37 ms`; service smokes cover Qdrant, two-service sharded Qdrant, and pgvector. | The 10M and 50M FAISS results are compressed target-recall profiles, not exact-neighbor recall. The unfinished strict profiles are 10M Qdrant, 10M sharded Qdrant, 10M pgvector, and 100M sharded Qdrant. |
...
| Production streaming load runner | Memory-bounded large-N runner that generates and inserts vectors in batches and measures target-recall, p99, SLO, and cost without storing the full corpus or exact-neighbor matrix in RAM. | implemented | FAISS persisted / FAISS IVF-PQ persisted / Qdrant service streaming / Qdrant sharded service streaming / pgvector streaming | Keep the passing 10M and 50M compressed FAISS profiles green, then complete 10M Qdrant/pgvector service evidence and the real 100M distributed sharded-Qdrant run. |
...
- The checked Kubernetes network drill writes `256` deterministic memories
  ...
  [GitHub Actions run](https://github.com/CaspianG/wavemind/actions/runs/29057247023).
- The checked active-active Kubernetes drill uses three separate replicated
  ...
  [GitHub Actions run](https://github.com/CaspianG/wavemind/actions/runs/29058433643).
