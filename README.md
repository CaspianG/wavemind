...
| Redis hot cache | Redis-compatible shared cache is visible across workers, query-audit prewarm warms `1` hot query, Memory OS warms `2` observed hot queries plus `5` predictive neighbor queries, applies `7` useful/not-useful recall feedback events, demotes cold memories, cross-worker hit `true`, namespace invalidation `true`, production architecture advice `architecture_required`. |
...
Redis-compatible shared cache behavior, Memory OS shared prewarm, explicit useful/not-useful recall feedback, batch feedback updates, predictive prefetch, production architecture advice, and namespace
