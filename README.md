...
| Sustained HTTP cluster load | 4 real localhost API nodes, RF=3, 8 quorum writes, 8 normal queries, 8 failover queries, 4 deletes, success rate `1.000`, failover hit rate `1.000`, delete suppression `1.000`, repair copied `1` missing replica, p99 operation `441.52 ms`. |
...
delta sync, field-only hotness delta sync, field-state CRDT convergence,
...
| 10M memory-scale profile | Checked-in compressed FAISS IVF-PQ streaming profile exists and is reported in the generated leaderboard. | `benchmarks/production_streaming_load_ivfpq_10m_results.json` | Not yet a 10M Qdrant/pgvector service comparison. |
...
| Qdrant streaming | Real Qdrant streaming smoke exists, a tuned 1M service run passes SLO after warmup/chunking, and a 10M plan-only service contract is checked in. | `benchmarks/production_streaming_load_qdrant_smoke_results.json`, `benchmarks/production_streaming_load_qdrant_1m_results.json`, `benchmarks/production_streaming_load_qdrant_1m_tuned_results.json`, `benchmarks/production_streaming_load_qdrant_10m_plan.json` | The 10M Qdrant service result is not claimed until `production_streaming_load_qdrant_10m_results.json` is produced by a real run. |
...
| Production readiness gate | Current WaveMind core gate score is `1.000`: `35/35` criteria pass, `0` require action, `0` fail. Live Zep competitor evidence is tracked separately and remains pending until a real `ZEP_API_URL` or `ZEP_API_KEY` is configured. | This keeps production claims honest without letting a missing commercial competitor credential block WaveMind's own readiness verdict. WaveMind has a real production foundation, a checked-in 10M compressed FAISS profile, a checked 50M run preflight, a measured pgvector tuning path, real Qdrant and pgvector streaming service smokes plus 10M preflights, a real tuned 1M Qdrant streaming SLO pass, a deterministic 100M capacity envelope, real local multi-process HTTP cluster smoke, VectorDBBench custom dataset export, architecture advisor preflight, cluster autoscale planning, and majority control-plane safety. |
...
- The Qdrant streaming path has a real service smoke and a checked 10M preflight contract. It is not a completed 10M Qdrant benchmark until `benchmarks/production_streaming_load_qdrant_10m_results.json` is produced by a real run.
