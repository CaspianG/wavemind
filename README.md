...
`benchmarks/serverless_observed_telemetry_benchmark.py`: it starts a real local
WaveMind HTTP API worker, warms the hot-query cache, measures p95/p99/error
rate/cold start, and multiplies per-replica throughput by `max_scale=256` for
the Knative/KEDA horizontal capacity estimate. This is loopback evidence, not a
real-cluster performance claim; the same contract should be rerun against a
real Knative/KEDA deployment before publishing managed serverless numbers.
