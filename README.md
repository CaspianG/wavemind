...
| Redis hot cache | Redis-compatible shared cache is visible across workers, query-audit prewarm warms `1` hot query, Memory OS warms `2` observed hot queries plus `5` predictive neighbor queries, demotes cold memories, cross-worker hit `true`, namespace invalidation `true`. |
...
| Sustained HTTP cluster load | 4 real localhost API nodes, RF=3, 8 quorum writes, 8 normal queries, 8 failover queries, 4 deletes, success rate `1.000`, failover hit rate `1.000`, delete suppression `1.000`, repair copied `1` missing replica, p99 operation `216.35 ms`. |
...
Redis-compatible shared cache behavior, Memory OS shared prewarm, predictive prefetch, and namespace
...
checks index health, and returns
operator-facing recommendations. The cluster-health command probes every
