...
| Precomputed multimodal storage contract | Runner-ready path for validating explicitly supplied shared-space vectors, query vectors, `s3://` asset URIs, checksums, persistence, provenance, and routing. This is integration evidence, not encoder-quality evidence. | Optional `benchmarks/multimodal_external_encoder_results.json` | `wavemind multimodal-external-evidence --manifest external_multimodal_manifest.json --write-artifacts --output benchmarks/multimodal_external_encoder_results.json`; this artifact cannot unlock real-encoder admission. |
