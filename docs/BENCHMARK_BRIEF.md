...
|---|---|---|---|
| Adaptive agent-memory advantage | Across 7 identical-protocol trials, WaveMind Core reaches task success `0.389`, stale error `0.611`, and combined score `0.406`; WaveMind + Memory OS reaches `1.000`, `0.000`, and `0.925`. The paired task-success lift is `0.611` with 95% interval `[0.389, 0.833]`; p95 retrieval latency improves from `1.72 ms` to `1.61 ms`. Real Chroma and Qdrant baselines are included; unavailable external engines are explicitly skipped. | `benchmarks/agent_memory_advantage_results.json`, `benchmarks/agent_memory_advantage_admission_results.json`, `benchmarks/AGENT_MEMORY_ADVANTAGE_ADMISSION.md` | `python benchmarks/agent_memory_advantage_benchmark.py --output benchmarks/agent_memory_advantage_results.json`; `wavemind agent-memory-advantage-admission --expected-source-sha <artifact-source-sha> --fail-on-blocked` |
...
| Experienced Work Agent | On 60 verified training trajectories and 30 frozen held-out coding, support, and enterprise tasks, typed experience raises task success from WaveMind Core's `0.167` to `1.000`, reduces repeated observed errors from `0.833` to `0.000`, median tool steps from `4` to `3`, and median context from `60` to `36` tokens. The separate admission is `12/12`; this remains controlled local product evidence. | `benchmarks/experienced_work_agent_results.json`, `benchmarks/EXPERIENCED_WORK_AGENT.md`, `benchmarks/experience_quality_admission_results.json`, `benchmarks/EXPERIENCE_QUALITY_ADMISSION.md` | `python benchmarks/experienced_work_agent_benchmark.py`; `wavemind experience-quality-admission --expected-source-sha <artifact-source-sha> --fail-on-blocked` |
| Developer experience admission | A clean local run creates Python, TypeScript, MCP, and Docker starters, produces the first cited Experience Packet in `0.58s`, returns the same citation after restart, passes all required doctor checks, preserves unrelated files, and validates generated syntax/configuration. Admission is `8/8`; the Docker runtime is tested separately by full-check CI. | `benchmarks/developer_experience_admission_results.json`, `benchmarks/DEVELOPER_EXPERIENCE_ADMISSION.md` | `python benchmarks/developer_experience_admission.py --fail-on-blocked` |
| Memory safety admission | The frozen suite contains 375 attacks and 25 benign controls across direct and indirect injection, delayed payloads, tenant isolation, poisoned workflows, malicious corrections, multimodal metadata, taint propagation, protected deletion, and trust escalation. Admission is `10/10`: attack success `0.00`, benign acceptance `1.00`, zero namespace leakage or untrusted promotion, and 25/25 exact SQLite rollback/provenance checks over three identical runs. | `benchmarks/memory_safety_admission_results.json`, `benchmarks/MEMORY_SAFETY_ADMISSION.md` | `wavemind memory-safety-admission --expected-source-sha <source-sha> --write-artifacts --fail-on-blocked` |
| Provider integration admission | The frozen suite executes 11 mandatory Python, OpenAI Agents, Anthropic, MCP, LangGraph, HTTP, portability, Mem0-import, and packed TypeScript SDK cases. Admission is `10/10`: all three runs pass with one verdict fingerprint, Experience Packet citation parity is `1.00` across five provider surfaces, portable replay parity is `1.00`, and the clean TypeScript package passes a live memory lifecycle, safe retry, cancellation, mutation, and concurrency checks. | `benchmarks/integration_admission_results.json`, `benchmarks/INTEGRATION_ADMISSION.md` | `wavemind integration-admission --expected-source-sha <source-sha> --write-artifacts --fail-on-blocked` |
| Agent impact leaderboard | Aggregates checked-in behavioral evidence across `8` benchmark groups, including the controlled adaptive-memory and Experienced Work Agent runs. WaveMind currently has `8/8` primary wins over the best non-WaveMind baseline inside each artifact, average primary lift `0.412`, average context saved `0.657`, and average stale-safety score `0.924`; Core rows remain visible rather than being filtered out. | `benchmarks/agent_impact_results.json`, `benchmarks/AGENT_IMPACT.md` | `python benchmarks/agent_impact_leaderboard.py --output benchmarks/agent_impact_results.json --markdown-output benchmarks/AGENT_IMPACT.md` |
...
| Memory OS intelligence report | Pulls adaptive-worker evidence out of scale readiness, agent coherence, canary, admission, and policy-bundle artifacts into one public report. It has `39/39` gate checks passing across hot-query prewarm, transition-learned predictive prefetch, priority learning, adaptive forgetting, concept consolidation, Redis coordination, context savings, canary controls, and the admitted six-hour/two-worker production topology. Broader multi-region and 100M claims remain separate gates. | `benchmarks/memory_os_intelligence_results.json`, `benchmarks/MEMORY_OS_INTELLIGENCE.md` | `python benchmarks/memory_os_intelligence_report.py --output benchmarks/memory_os_intelligence_results.json --markdown-output benchmarks/MEMORY_OS_INTELLIGENCE.md` |
...
| LoCoMo direct Memory OS | Full official `locomo10.json`: 1977 evidence questions and 5882 memories. Core precision@1 is `0.2387`; direct feedback-free Memory OS is `0.2382`, a small regression within the one-point admission tolerance. The result is admitted as complete execution, not claimed as uplift. | `benchmarks/locomo_memory_os_results.json` | `python benchmarks/locomo_memory_benchmark.py --dataset benchmarks/data/locomo10.json --engines wavemind wavemind-memory-os --output benchmarks/locomo_memory_os_results.json` |
...
| LongMemEval-S direct Memory OS | Full 470-question run over 231595 turn memories. Core and direct Memory OS both reach recall@5 `0.5990`, precision@1 `0.4489`, and MRR@5 `0.5619`; Memory OS completes 470 worker cycles with zero worker errors. | `benchmarks/longmemeval_memory_os_results.json` | `python benchmarks/longmemeval_memory_benchmark.py --dataset benchmarks/data/longmemeval_s_cleaned.json --granularity turn --engines wavemind wavemind-memory-os --output benchmarks/longmemeval_memory_os_results.json` |
| LongMemEval-V2 Small | The frozen full 451-question protocol now uses official per-question haystacks, isolated A/B stores, all 29 image questions, and local `qwen3.5:9b`. Core reaches `0.1863`; Memory OS reaches `0.1818` while saving `41.00%` of Core context and adding `1.59 ms` to p95. The untouched remaining 419 show `0.1885` versus `0.1766`. This architecture is recorded as a failed experiment because it regresses quality and improves only three categories; it is not admission evidence. A final exact-state dev32 hypothesis also failed the frozen go/no-go gate: `0.1875` versus `0.1875`, despite `36.17%` context saving and a bounded `+2.11 ms` p95 delta. No second full run was launched. The older `longmemeval_v2_small_per_query.jsonl` and frozen-20 artifacts remain historical protocol evidence only. | `benchmarks/longmemeval_v2_hybrid1150_full451_results.json`, `benchmarks/longmemeval_v2_hybrid1150_full451_per_query.jsonl`, `benchmarks/longmemeval_v2_hybrid1150_split_analysis.json`, `benchmarks/LONGMEMEVAL_V2_HYBRID1150_SPLIT_ANALYSIS.md`, `benchmarks/longmemeval_v2_exact_state_dev32_results.json`, `benchmarks/longmemeval_v2_exact_state_dev32_per_query.jsonl`, `benchmarks/LONGMEMEVAL_V2_EXACT_STATE_DEV32.md` | `python benchmarks/longmemeval_v2_split_analysis.py --full-results benchmarks/longmemeval_v2_hybrid1150_full451_results.json --full-per-query benchmarks/longmemeval_v2_hybrid1150_full451_per_query.jsonl --development-results benchmarks/longmemeval_v2_hybrid1150_dev32_results.json --development-per-query benchmarks/longmemeval_v2_hybrid1150_dev32_per_query.jsonl --output benchmarks/longmemeval_v2_hybrid1150_split_analysis.json --markdown-output benchmarks/LONGMEMEVAL_V2_HYBRID1150_SPLIT_ANALYSIS.md` |
...
| Production streaming load runner | Memory-bounded runner for 10M/50M/100M profiles. Strict checked results include 50M FAISS IVF-PQ at recall@10 `0.9705`, p99 `73.11 ms`; single-service 10M Qdrant at `0.975`, `43.27 ms`; four-service 10M sharded Qdrant at `0.9925`, `71.28 ms`; and four-service 10M pgvector at `0.975`, `87.66 ms`. The remote 100M workflow requires eight unique machines across three or more regions, 16 GB RAM and 35 GB disk per shard, private Qdrant endpoints through pinned SSH tunnels, resumable checkpoints, and same-run topology attestation. The checked-in 100M plan remains a contract, not a completed benchmark. | `benchmarks/production_streaming_load_qdrant_smoke_results.json`, `benchmarks/production_streaming_load_ivfpq_50m_results.json`, `benchmarks/production_streaming_load_qdrant_10m_plan.json`, `benchmarks/production_streaming_load_qdrant_sharded_10m_plan.json`, `benchmarks/production_streaming_load_qdrant_sharded_100m_plan.json`, `benchmarks/production_streaming_load_pgvector_10m_plan.json`, `benchmarks/production_streaming_load_qdrant_10m_results.json`, `benchmarks/production_streaming_load_qdrant_sharded_10m_results.json`, `benchmarks/production_streaming_load_pgvector_10m_results.json`, `deploy/remote-scale`, `.github/workflows/production-streaming-load.yml`, `.github/workflows/remote-qdrant-100m-lab.yml` | `python benchmarks/production_streaming_load_benchmark.py --plan-only --runner-storage-root state/production-runs --disk-free-gb 0 --sizes 10000000 --engines pgvector-service`; `gh workflow run remote-qdrant-100m-lab.yml -f action=evidence -f runner_label=self-hosted-large` |
...
| External HTTP active-active runner | Runner-ready benchmark for real WaveMind API regions. It checks namespace-delta export/import, convergence, delete propagation, cursor idempotency, final no-op sync, p99, and `slo_pass`. `.github/workflows/remote-production-lab.yml` additionally attests three unique machines, deploys the production stack, physically stops one region API, verifies survivor writes/tombstones, restarts the failed region, and requires final convergence. `deploy/cloud/gcp-remote-active-active` reproducibly provisions the three-region GCE inventory but is not evidence until the workflow passes. Strict admission requires both measured artifacts; a plan or transport-only result cannot unlock the claim. | optional `benchmarks/external_http_active_active_results.json` plus `benchmarks/remote_active_active_failure_drill_results.json` | `python deploy/remote/remote_lab.py failure-drill --inventory state/remote-inventory.json --failed-region eu-west --namespace-count 16 --output state/remote_active_active_failure_drill_results.json` |
...
| Production evidence ingest gate | Maintainer review path for downloaded remote/large-N artifacts. It validates remote HTTP cluster, active-active, managed/serverless, Qdrant, sharded Qdrant, pgvector, FAISS IVF-PQ, and 100M result files through the same strict rules. Active-active requires its physical failure companion; 100M requires same-run eight-host attestation. | `wavemind/production_evidence_ingest.py`, `benchmarks/ingest_production_evidence_artifacts.py`, `tests/test_production_evidence_ingest.py` | `wavemind ingest-production-evidence --artifact-dir state/large-run --refresh`; `python benchmarks/ingest_production_evidence_artifacts.py --artifact-dir state/large-run --dry-run` |
...
| Active-active admission gate | Deployment-facing gate for remote multi-region active-active rollout. It joins strict transport evidence, physical region failure/recovery, and preflight state for `external_http_active_active`; local/loopback or transport-only evidence cannot unlock the production claim. | `benchmarks/active_active_admission_results.json`, `benchmarks/ACTIVE_ACTIVE_ADMISSION.md` | `wavemind active-active-admission --allow-plan-only --write-artifacts`; `wavemind active-active-admission --fail-on-blocked` |
| Serverless admission gate | Deployment-facing gate for managed/serverless rollout. It requires provider control-plane identity, Git/workflow provenance, measured cold start and scale-out, at least 1000 successful requests, provider request/latency/startup/instance metrics, and actual pool RPS without max-scale extrapolation. | `benchmarks/serverless_admission_results.json`, `benchmarks/SERVERLESS_ADMISSION.md`, `.github/workflows/managed-serverless-cloud-run.yml` | `wavemind serverless-admission --allow-plan-only --write-artifacts`; `wavemind serverless-admission --fail-on-blocked` |
| Precomputed multimodal storage contract | Runner-ready path for validating explicitly supplied shared-space vectors, query vectors, `s3://` asset URIs, checksums, persistence, provenance, and routing. This is integration evidence, not encoder-quality evidence. | Optional `benchmarks/multimodal_precomputed_contract_results.json` | `wavemind multimodal-external-evidence --manifest external_multimodal_manifest.json --write-artifacts`; its default output is isolated from the admitted real-encoder artifact and cannot unlock admission. |
| Real local multimodal benchmark | Pinned SentenceTransformers, CLIP, CLAP, and OpenShape PointBERT encoders process 1000 public text/image/audio/video/3D assets and 200 held-out queries. Three exact-SHA runs reach macro/cross-modal/mixed precision@1 `0.925`, parity `1.000`, retrieval p99 `48.64 ms`, and zero errors. | `benchmarks/multimodal_external_encoder_results.json`, `benchmarks/multimodal_per_query.jsonl`, `benchmarks/multimodal_per_asset.jsonl` | Evidence is dataset-, model-, SHA-, and topology-bounded; public source assets are referenced by pinned provenance rather than redistributed. |
| Multimodal admission gate | Deployment-facing gate for production multimodal memory claims. It requires the structured-memory contract plus real local text/image/audio/video/3D encoders, at least 1000 real/public assets, 200 independent queries, explicit shared spaces, bidirectional cross-modal checks, per-modality quality and latency budgets, leakage protection, reproducible evidence, and a verified local MinIO lifecycle. | `benchmarks/multimodal_admission_results.json`, `benchmarks/MULTIMODAL_ADMISSION.md` | Current exact-SHA status is `admitted`; reproduce with `wavemind multimodal-admission --fail-on-blocked --write-artifacts`. Use `wavemind multimodal-admission --allow-plan-only` only for preflight reporting when evidence is absent. |
| Agent-memory advantage admission gate | Strict release-facing gate joining the admitted controlled adaptive-memory comparison with direct full LoCoMo, LongMemEval-S, and LongMemEval-V2 evidence. The public gate is blocked until V2 is rerun on all 451 questions with official haystacks, isolated stores, at least `0.18` task success, at least `0.01` Memory OS uplift, four improved categories, and end-to-end p95 within both `20%` and `5 ms`. | `benchmarks/agent_memory_advantage_admission_results.json`, `benchmarks/AGENT_MEMORY_ADVANTAGE_ADMISSION.md` | `wavemind agent-memory-advantage-admission --expected-source-sha <artifact-source-sha> --fail-on-blocked` |
...
- an independently verified LoCoMo, LongMemEval, MTEB, MIRACL, RAGBench, or
  VectorDBBench leaderboard placement;
