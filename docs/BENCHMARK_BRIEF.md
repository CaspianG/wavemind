...
| Memory competitor adapter profile | WaveMind reaches `precision@1 0.80`, `precision@3 1.00`, stale suppression `1.00`; Mem0 runs locally with Qdrant + FastEmbed and reaches `0.80`, `1.00`, stale suppression `0.60`; LangGraph persistent SQLite reaches `0.80`, `1.00`, stale suppression `1.00`; Zep has a live `zep-python` adapter path and remains skipped until `ZEP_API_URL` or `ZEP_API_KEY` points at a real service. | `benchmarks/memory_competitor_results.json` | `python benchmarks/memory_competitor_benchmark.py --engines wavemind mem0 zep langgraph` |
