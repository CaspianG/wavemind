...
| LongMemEval-V2 Small | The earlier 451-question run remains full execution history, but it predates official per-question haystack filtering and isolated A/B stores. The strict frozen-20 protocol uses both controls with pinned local `qwen2.5:3b`; Core and Memory OS each reach `0.10`, so the result is a blocked quality smoke, not uplift evidence. | `benchmarks/longmemeval_v2_frozen20_protocol_results.json`, `benchmarks/longmemeval_v2_frozen20_protocol_per_query.jsonl`, `benchmarks/longmemeval_v2_small_memory_os_results.json`, `benchmarks/longmemeval_v2_small_per_query.jsonl` | `python benchmarks/longmemeval_v2_memory_benchmark.py --data-root <pinned-dataset> --limit-questions 20 --ollama-model qwen2.5:3b --ollama-text-context-window 8192 --output benchmarks/longmemeval_v2_frozen20_protocol_results.json --per-query-output benchmarks/longmemeval_v2_frozen20_protocol_per_query.jsonl` |
