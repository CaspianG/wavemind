...
| Production index profile | At 50000 vectors, persisted FAISS and Qdrant service both reach `recall@10 1.000`; pgvector with `ef_search=400` reaches `0.811`. The runner also exposes `pgvector-exact` for recall audits and `pgvector-iterative` for HNSW + filtered-collection tuning. | `benchmarks/production_index_profile_results.json` | `docker compose -f examples/production-index-profile/docker-compose.yml run --rm benchmark` |
...
| Production streaming load runner | Memory-bounded runner for 10M/50M profiles. The checked-in 10M compressed FAISS IVF-PQ run reaches target recall@10 `0.990`, p99 `60.13 ms`, and valid SLO/cost status; the checked-in 50M plan-only artifact estimates a `1.12 GB` compressed FAISS IVF-PQ index, `119.209 GB` application storage, and the exact future result command without claiming a completed benchmark; the 100k IVF-PQ smoke reaches `0.960`, p99 `1.10 ms`; the 10k numpy smoke reaches `1.000`, p99 `0.98 ms`. | `benchmarks/production_streaming_load_ivfpq_10m_results.json`, `benchmarks/production_streaming_load_50m_plan.json`, `benchmarks/production_streaming_load_ivfpq_100k_results.json`, `benchmarks/production_streaming_load_smoke_results.json` | `python benchmarks/production_streaming_load_benchmark.py --plan-only --sizes 50000000 --engines faiss-ivfpq-persisted --output benchmarks/production_streaming_load_50m_plan.json --planned-result-output benchmarks/production_streaming_load_ivfpq_50m_results.json` |
...
| Production readiness gate | Current WaveMind core gate score is `1.000`: `31/31` criteria pass, `0` require action, `0` fail. Live Zep competitor evidence is tracked separately and remains pending until a real service is configured. | `benchmarks/production_readiness_results.json`, `benchmarks/PRODUCTION_READINESS.md` | `python benchmarks/production_readiness_gate.py --output benchmarks/production_readiness_results.json --markdown-output benchmarks/PRODUCTION_READINESS.md` |
...
- that pgvector is already a recommended production candidate index for
  ...
  target; the new `pgvector-exact` and `pgvector-iterative` runner modes are
  the path to separate correctness loss from HNSW tuning loss;
