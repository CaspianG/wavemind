...
| LongMemEval-V2 Small | Full 451-question answer-quality run with all 29 image questions and pinned local `qwen2.5vl:3b`. Core task success is `0.0754`; Memory OS reaches `0.0909`, with retrieval p95 `13.88 ms`, 15 worker runs, and zero errors. Absolute answer quality remains low. | `benchmarks/longmemeval_v2_small_memory_os_results.json`, `benchmarks/longmemeval_v2_small_per_query.jsonl` | `python benchmarks/longmemeval_v2_memory_benchmark.py --data-root <pinned-dataset> --ollama-model qwen2.5vl:3b --ollama-vision-model qwen2.5vl:3b --reader-supports-images --output benchmarks/longmemeval_v2_small_memory_os_results.json --per-query-output benchmarks/longmemeval_v2_small_per_query.jsonl` |
...
| Agent-memory advantage admission gate | Strict release-facing gate joining the controlled adaptive-memory comparison with direct full LoCoMo, LongMemEval-S, and LongMemEval-V2 Small execution. Current status is `admitted`, `13/13`; missing exact-SHA artifacts, incomplete public runs, worker errors, stale regressions, insufficient context savings, or latency regressions block the claim. | `benchmarks/agent_memory_advantage_admission_results.json`, `benchmarks/AGENT_MEMORY_ADVANTAGE_ADMISSION.md` | `wavemind agent-memory-advantage-admission --expected-source-sha <artifact-source-sha> --fail-on-blocked` |
