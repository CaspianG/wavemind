...
| Agent impact leaderboard | Aggregates checked-in behavioral evidence across `7` benchmark groups, including the controlled adaptive-memory advantage run. WaveMind currently has `7/7` primary wins over the best non-WaveMind baseline inside each artifact, average primary lift `0.374`, average context saved `0.657`, and average stale-safety score `0.924`; Core rows remain visible rather than being filtered out. | `benchmarks/agent_impact_results.json`, `benchmarks/AGENT_IMPACT.md` | `python benchmarks/agent_impact_leaderboard.py --output benchmarks/agent_impact_results.json --markdown-output benchmarks/AGENT_IMPACT.md` |
