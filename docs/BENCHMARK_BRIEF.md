...
| Production streaming load runner | Memory-bounded runner for 10M/50M/100M profiles. The checked-in 10M compressed FAISS IVF-PQ run reaches target recall@10 `0.990`, p99 `60.13 ms`, and valid SLO/cost status; the real Qdrant service smoke reaches target recall@10 `1.000`, p99 `17.90 ms`; the cold 1M Qdrant streaming run reaches recall@10 `0.990` but fails p99 at `3013.98 ms`; the tuned 1M Qdrant run with safe upsert chunks, wait-after-build, and 100 warmup queries reaches target recall@10 `1.000`, p99 `26.37 ms`, and valid SLO/cost status; the real two-service sharded Qdrant smoke reaches target recall@10 `1.000`, p99 `16.02 ms`; the real PostgreSQL/pgvector service smoke reaches target recall@10 `1.000`, p99 `7.62 ms`; the checked-in single-service Qdrant, sharded Qdrant, and pgvector 10M plan-only artifacts include explicit runner storage roots for checkpoints and local index paths; the checked-in 50M FAISS IVF-PQ plan-only artifact estimates a `1.12 GB` compressed FAISS IVF-PQ index, `119.209 GB` application storage, and the exact future result command; the checked-in 100M sharded Qdrant plan is a resumable service-run contract, not a completed benchmark. | `benchmarks/production_streaming_load_ivfpq_10m_results.json`, `benchmarks/production_streaming_load_qdrant_smoke_results.json`, `benchmarks/production_streaming_load_qdrant_1m_results.json`, `benchmarks/production_streaming_load_qdrant_1m_tuned_results.json`, `benchmarks/production_streaming_load_qdrant_sharded_smoke_results.json`, `benchmarks/production_streaming_load_qdrant_10m_plan.json`, `benchmarks/production_streaming_load_qdrant_sharded_10m_plan.json`, `benchmarks/production_streaming_load_qdrant_sharded_100m_plan.json`, `benchmarks/production_streaming_load_pgvector_smoke_results.json`, `benchmarks/production_streaming_load_pgvector_10m_plan.json`, `benchmarks/production_streaming_load_50m_plan.json` | `.github/workflows/production-streaming-load.yml`; `docker compose -f examples/qdrant-sharded-streaming/docker-compose.yml up -d`; `python benchmarks/production_streaming_load_benchmark.py --plan-only --runner-storage-root state/production-runs --disk-free-gb 0 --sizes 10000000 --engines qdrant-service --output benchmarks/production_streaming_load_qdrant_10m_plan.json --planned-result-output benchmarks/production_streaming_load_qdrant_10m_results.json`; `python benchmarks/production_streaming_load_benchmark.py --plan-only --runner-storage-root state/production-runs --disk-free-gb 0 --sizes 10000000 --engines qdrant-sharded-service --output benchmarks/production_streaming_load_qdrant_sharded_10m_plan.json --planned-result-output benchmarks/production_streaming_load_qdrant_sharded_10m_results.json`; `python benchmarks/production_streaming_load_benchmark.py --plan-only --runner-storage-root state/production-runs --disk-free-gb 0 --sizes 10000000 --engines pgvector-service --output benchmarks/production_streaming_load_pgvector_10m_plan.json --planned-result-output benchmarks/production_streaming_load_pgvector_10m_results.json` |
