...
| Production streaming load runner | Memory-bounded runner for 10M/50M service-backed profiles. The checked-in smoke run uses 10000 vectors, target recall@10 `1.000`, p99 `0.98 ms`, and SLO `pass`; it is a format/regression smoke, not a 10M claim. | `benchmarks/production_streaming_load_smoke_results.json` | `python benchmarks/production_streaming_load_benchmark.py --sizes 10000000 --engines faiss-persisted qdrant-service --output benchmarks/production_load_10m_results.json` |
...
| Production readiness gate | Current gate score is `0.867`: `13/15` criteria pass, `2` require action, `0` fail. | `benchmarks/production_readiness_results.json`, `benchmarks/PRODUCTION_READINESS.md` | `python benchmarks/production_readiness_gate.py --output benchmarks/production_readiness_results.json --markdown-output benchmarks/PRODUCTION_READINESS.md` |
| Memory competitor adapter profile | WaveMind reaches `precision@1 0.80`, `precision@3 1.00`, stale suppression `1.00`; Mem0 runs locally with Qdrant + FastEmbed and reaches `0.80`, `1.00`, stale suppression `0.60`; LangGraph persistent SQLite reaches `0.80`, `1.00`, stale suppression `1.00`; Zep remains skipped until a real Zep service/API key and cleanup policy are configured. | `benchmarks/memory_competitor_results.json` | `python benchmarks/memory_competitor_benchmark.py --engines wavemind mem0 zep langgraph` |
...
- that the production streaming smoke is a 10M-vector performance result. It
  proves the memory-bounded runner and artifact shape; the real 10M run still
  needs a service-backed FAISS/Qdrant/pgvector environment sized for the index;
