...
Этот файл хранит черновики для русскоязычных площадок: VC, Telegram, Habr,
личные блоги и короткие посты. Цель: объяснить WaveMind простым языком, без
завышенных обещаний и без ощущения заказного комментария.
...
Обычный векторный поиск отвечает: "что похоже на запрос?" WaveMind добавляет
...
Проблема простая: большинство систем памяти для AI сейчас устроены как
обычный поиск.

Пользователь что-то сказал.
Мы сделали embedding.
Положили его в векторную базу.
Потом нашли ближайшие куски текста.

Это работает, но у такой памяти нет ощущения времени и важности.

Например:

- пользователь раньше говорил, что его зовут Андрей;
- потом уточнил, что в проекте он хочет использовать другое имя;
- старый факт не должен всё время всплывать наравне с новым;
- временная информация должна исчезнуть;
- часто используемая информация должна становиться "горячее";
...
- SQLite хранит текст, метаданные, вектора и состояние памяти;
- обычный vector search находит кандидатов;
- динамический слой учитывает hotness, TTL, priority, decay, namespace и tags;
- поверх этого есть CLI, Python API и FastAPI сервер;
- есть интеграции с LangChain, LangGraph, LlamaIndex, CrewAI и AutoGen.

Это не замена Chroma, Qdrant или Pinecone. Скорее слой памяти рядом с ними или
поверх них. Векторная база хорошо отвечает на вопрос "что похоже?", а
WaveMind пытается добавить вопрос "что всё ещё важно?"

Самое важное для меня сейчас - не красивые слова про "волны", а проверяемые
бенчмарки. В репозитории уже есть результаты по dynamic memory, LoCoMo,
LongMemEval, BEIR/SciFact и локальным ANN-тестам. Там честно написано, где
WaveMind сильнее, а где пока проигрывает по скорости.
...
Мне особенно интересна обратная связь от тех, кто делает:

- долгоживущих AI-агентов;
- локальные AI-приложения;
- RAG-системы;
- персональных ассистентов;
- support/CRM-ботов;
- исследовательские или trading-инструменты.
...
WaveMind - open-source попытка сделать такой слой памяти: SQLite, vector
search, hotness, decay, TTL, namespaces и audit log.
...
Он забывает предпочтения, тянет старые факты, путает пользователей и не
понимает, что временная информация уже устарела.
