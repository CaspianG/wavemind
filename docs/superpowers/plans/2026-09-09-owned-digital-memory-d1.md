...
- Reported-procedure extraction belongs only to the Brain private adapter, not the frozen-quality legacy `AgentExperienceRuntime`. Task9 full-suite diagnosis supersedes the earlier optional `declared_procedure` extension to the shared runtime: restore its exact pre-D1 implementation and isolate the new reported-state candidate path in `wavemind/brain/experience_runtime_bridge.py`. Retain honest 'Reported steps' / 'Procedure reported for a verified outcome' language, not 'Operator-declared' when an agent supplied the report, and no invented TOOL_CALL events or observed-tool applicability. Outcome verification does not establish observation of each step or causal benefit. PrivateRuntime alone may instantiate a narrow Brain-specific subclass that reuses the real unchanged finalize/compiler/firewall/promotion behavior. Preserve existing Brain candidate IDs, canonical fingerprint fields, content, verification/source/provenance, idempotent replay and purge semantics. The base runtime must ignore this Brain-only metadata; no global monkeypatch, new allowlisted blob, validator exception or changed promotion threshold is allowed. Add actual base-versus-Brain regression and rerun amended experience/recovery/context/UI/transport and the three previously failing admissions before full clean-candidate verification.
...
**Files:** Create `wavemind/brain/experience_bridge.py`, `experience_records.py`, `experience_runtime_bridge.py` and corresponding focused `tests/brain` modules; extend service, context provider hook, lifecycle cleanup and narrow BrainStore manifest migration/tests. The final reported-procedure path is Brain-private; `wavemind/experience_runtime.py` remains byte-identical to pre-D1 after the Task9 isolation correction, with base/private behavior covered by tests.
...
### Controller refinement: trusted runtime outbox maintenance

Actual Task8 P1 browser execution reached restart, correction, two bound clients and three independent owner attestations, but verified outcomes remained pending: existing transport servers never schedule the domain outbox drain. Domain-only tests explicitly drained it. The production lifecycle must deliver that already-approved integration contract without a test-only shortcut.

- Add one shared small `wavemind/brain/maintenance.py` lifecycle if needed, consumed by HTTP `mount_brain` and the real MCP server lifespan. Do not start threads in `BrainService.__init__`, add maintenance request endpoints, extend client authority, or change storage schema/bridge admission semantics.
- Each launched server owns one serial worker: call existing `service.drain_outbox(limit=100)` off the event loop, starting with a startup batch, then wait 1 second between batches, including failures. Reuse the existing transaction/live-authority/cleanup-first/idempotent acknowledgment logic. The batch count is bounded; do not claim a hard wall-clock drain limit or cross-database atomicity.
- Ordinary drain failures remain durable pending work and are retried; sanitized fixed diagnostics only, no source content, secrets or trace locals. Never report pending work as integrated. Do not spawn overlapping batches or busy-loop while a failure persists.
- On shutdown signal stop, await any in-flight drain and worker completion, and only then close service/auth resources. Do not cancel or abandon a thread and close its SQLite connection underneath it. Preserve existing application/MCP lifespans and shutdown handlers; domain callers retain explicit maintenance control.
- Observe focused RED/GREEN for actual server lifespan integration of queued work, recovery after one transient maintenance failure, pending startup/restart, and shutdown waiting for an in-flight batch before resource close. Include the actual standalone MCP lifecycle, not just a mock adapter. Reuse existing real transport/scenario tests where practical; a focused `tests/brain/test_maintenance.py` is allowed.
- The P1/B1 browser assertions must wait for real server-produced integration/current eligibility through ordinary authenticated routes, without direct fixture drains or optimistic state changes. Private learning thresholds and owner-versus-verifier labels remain unchanged; D1 local server maintenance is not an agent action or an external network/model call.

### Controller refinement: correction lineage is provenance, not a new temporal root

After real maintenance ran, P1's first newly verified post-correction outcome was blocked. The full immutable fingerprint still matched, but `experience_records.live_basis` independently treated every semantic in the all-origin closure as a current root. That re-imposed current-time validity on the superseded predecessor even though the existing shared reconciliation traversal intentionally checks replacement lineage non-temporally and ordinary prerequisites temporally.

- Allow the narrow fix in `wavemind/brain/experience_records.py` with focused regression tests (a separate focused test file is acceptable). Keep the schema, archive, immutable full basis fingerprint, source access/erasure provenance and shared reconciliation walker unchanged.
- Start from the exact semantic candidates in immutable `_basis_origins`. Within the same authorized transaction, inspect only those candidate payloads to identify explicit claim-to-claim `supersedes` edges whose target is another candidate. Remove those predecessor candidates only from the independently started root set, never from the basis/origins/fingerprint. Do not infer lineage from names, keys, source IDs, status alone or arbitrary relation payloads.
- Traverse every retained root using existing `reconcile.context_record_state`. It remains the authority for review/freshness, source permissions, effective bounds, ordinary prerequisites (including a predecessor also explicitly depended on), cycles/limits and complete origin collection. This root-set preparation is not a second dependency walker.
- Require every semantic pair from the original basis to be reached by retained-root traversals. A nonempty semantic basis with no roots, missing coverage, malformed lineage, cycle or inaccessible/missing record fails closed; do not allow vacuous eligibility. A source-only basis retains its existing behavior. Preserve transitions and pending handling for future-bound and budget-trimmed governing candidates.
- Observe RED/GREEN proving three new independent post-correction results can integrate and survive restart, while the old pre-correction receipt/experience remains blocked. Negative controls include a predecessor also used as an ordinary prerequisite, revoked/recheck-required lineage, a correction chain/reversed arrival order, missing coverage/cycles and future-bound/trimmed governing records. Keep existing context/reconciliation/experience/portability tests intact and run the full Brain coverage for this task.
...
- [ ] Run full pytest from clean test temp, Ruff, packaging build/twine/wheel asset smoke, docs local links/product-status sync, P1+B1 actual UI/HTTP/MCP twice with variations. Refresh relevant source-bound admissions via existing official scripts rather than rewriting claims. Record optional-dependency skips, failures and external gates separately; do not call the overall user mission 100% complete.
  The final archive-content regression builds a real sdist without installing dependencies during pytest. Declare `build>=1`, `setuptools>=68` and `wheel>=0.42` in the development extra so an ordinary fresh CI development install can execute it; these are test/build tools, not runtime requirements. Local verification may supply only missing build-tool packages through an isolated task-owned temporary module directory, with exact versions recorded and no shared interpreter mutation. Do not inject an unrelated bundled site-packages tree, add an optional skip, or hide a failed archive/browser check.
...
## Controller refinement: source-bound D1 evidence and required CI

Task9's runner launches real pytest in a fresh short temporary workspace and obtains per-test collection/setup/call/teardown outcomes through a dedicated collection hook. It captures the exact clean Git HEAD and tracked input hashes before and after execution; an expected-SHA mismatch or changed source cannot pass. Public `wavemind.brain_d1_evidence.v1` output contains sanitized repository-relative test IDs, actual outcomes, fixed skip classifications, runtime versions and complete F01–F14 results, not raw test stdout, source text, credentials, absolute profile paths or arbitrary exception messages. Failed or missing runs remain fail/unexecuted. A Brain-only execution mode must leave F13 unexecuted.

Each F gate names predeclared actual required tests, including all four real P1/B1 variants, real MCP, dependency/outcome/recovery and transition controls. A missing, skipped or failed required Brain test cannot pass its gates; collection errors and nonzero pytest execution cannot become success. Tests cover failure/skips/missing nodes/source changes and all lifecycle report stages, not only optimistic aggregation.

F13 is the full-suite **required compatibility coverage** verdict, not a promise that every optional integration was executed. It can pass after the complete suite succeeds and all predeclared required legacy cases execute, with only a closed, documented allowlist of existing optional dependency/platform skips. Those optional cases remain explicitly unexecuted/partial coverage in the report, including counts and fixed reasons. Do not accept all legacy skips, hide collection skips or turn absent tests into pass. Define allowed exact test/module identifiers and capability causes from existing tests before final execution; an unexpected new skip blocks F13 until investigated. For missing optional packages verify the named distribution is actually absent; a broken or API-incompatible installed distribution is not absence. Existing baseline16 skips and the upstream code provide evidence, not a dynamic accept-whatever-happened list. Windows privilege-only skips must name their actual bounded condition; other failures are not blanket platform exclusions. No100%-of-all-tests/all-integrations statement follows from a partial optional lane.

Add one reusable `.github/workflows/brain-d1.yml` with `workflow_call`, Python3.11, required Python test/MCP/provider dependencies, supported Node24, isolated pinned Playwright/Chromium tooling and explicit existing browser runtime overrides. It runs the full source-bound runner with actual checked-out GitHub SHA and uploads sanitized candidate-labelled evidence even on failure; missing/nonpass evidence fails the job. Use read-only contents permissions, no secrets, user profiles or publication. Pin/record actual browser tooling version; no unversioned production runtime dependency is introduced.
