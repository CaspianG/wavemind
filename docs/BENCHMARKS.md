...
| Memory OS quality gate | A direct sequential/adaptive A/B gives WaveMind baseline and WaveMind + Memory OS the same memories, observed queries, evaluation queries, and context shape. All `7/7` checks pass on the frozen 18-case development split: task success improves from `0.389` to `1.000`, stale errors fall from `0.611` to `0`, and both overall and cold p95 stay inside the `20%` and `5 ms` regression limits. | `benchmarks/memory_os_ab_results.json`, `benchmarks/memory_os_quality_results.json`, `benchmarks/MEMORY_OS_QUALITY.md` | Only this direct A/B is eligible as Memory OS uplift evidence. Current LoCoMo and LongMemEval runs remain supplemental until their runners execute Memory OS policies honestly. |
