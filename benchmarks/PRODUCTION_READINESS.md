...
|---|---|---|---|
| Checked-in benchmark artifacts are synchronized | `pass` | audit status pass, generated_at 2026-07-06T04:28:10Z | Keep the benchmark refresh workflow green and block stale artifacts before release. |
...
| Hot cache and query-audit prewarm work | `pass` | hit rate 0.92, prewarm hit True, p99 0.003500026650726795 ms | Back the cache with Redis in a service-mode benchmark. |
...
| Runtime replica quorum survives node loss | `pass` | recall after loss True, repair copied 1, p99 1.1847999994643033 ms | Measure the same path under concurrent writes and reads. |
