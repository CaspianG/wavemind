...
|---|---|---|---|
| Checked-in benchmark artifacts are synchronized | `pass` | audit status pass, generated_at 2026-07-07T11:33:07Z | Keep the benchmark refresh workflow green and block stale artifacts before release. |
...
| Snapshots, archives, offsite mirror, and object-store DR verify | `pass` | archive True, object-store DR True, restored files 3, PITR full True, PITR point True, journal entries 5, deleted 2 | Repeat the drill with real S3-compatible storage, larger SQLite journals, and database-native Postgres PITR runbooks. |
