...
|---|---|---|---|
| Checked-in benchmark artifacts are synchronized | `pass` | audit status pass, generated_at 2026-07-07T02:08:17Z | Keep the benchmark refresh workflow green and block stale artifacts before release. |
...
| Hot cache and query-audit prewarm work | `pass` | hit rate 0.92, prewarm hit True, p99 0.013100099749863148 ms | Keep local cache prewarm green while Redis carries multi-worker production cache evidence. |
...
| Sustained HTTP cluster load survives failover and repair | `pass` | nodes 4, writes 8, queries 8, failover hit 1.0, success 1.0, p99 220.00 ms | Repeat this profile against remote service nodes and larger namespace counts before claiming full distributed production scale. |
| Runtime replica quorum survives node loss | `pass` | recall after loss True, repair copied 1, p99 1.9653999479487538 ms, concurrent hit rate 1.0 | Extend the same replicated runtime profile to remote service nodes and sustained load. |
