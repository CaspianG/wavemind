...
|---|---|---|---|
| Checked-in benchmark artifacts are synchronized | `pass` | audit status pass, generated_at 2026-07-06T04:07:21Z | Keep the benchmark refresh workflow green and block stale artifacts before release. |
...
| Hot cache and query-audit prewarm work | `pass` | hit rate 0.92, prewarm hit True, p99 0.00300002284348011 ms | Back the cache with Redis in a service-mode benchmark. |
...
| HTTP shard transport handles failover, repair, and tombstones | `pass` | proxy bypass True, failover True, repair 1, tombstone deleted 1 | Extend the same HTTP shard profile with concurrent readers/writers and remote service nodes. |
| Runtime replica quorum survives node loss | `pass` | recall after loss True, repair copied 1, p99 1.5302000101655722 ms | Measure the same path under concurrent writes and reads. |
