...
|---|---|---|---|
| Checked-in benchmark artifacts are synchronized | `pass` | audit status pass, generated_at 2026-07-06T21:26:14Z | Keep the benchmark refresh workflow green and block stale artifacts before release. |
...
| LongMemEval answer generation beats static RAG baselines | `pass` | ollama qwen2.5:1.5b, queries 50, exact 0.240, contains 0.380, token F1 0.333, grounded 0.520, unsupported 0.000, abstain 0.480, evidence recall 0.920, retrieval 35.685 ms, Chroma F1 0.170, Qdrant F1 0.170 | Scale this from the checked 50-query local run to full LongMemEval-S with stronger local/API models and faithfulness scoring. |
