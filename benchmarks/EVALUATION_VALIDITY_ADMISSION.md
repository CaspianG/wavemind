...
Source SHA: `8b5295e70008b57a8b151b3abdcd0a67bc813e4e`

Rows: `5/16` implemented
...
| `positive-controls` | `blocked` | Oracle evidence or correct-state controls are executed. |
| `negative-controls` | `blocked` | Random, no-memory, stale, wrong-namespace, and deleted-evidence controls are executed. |
| `control-ordering` | `blocked` | Oracle is above a strong valid baseline, which is above random and no-memory; poison affects only its safety target. |
...
| `deterministic-verdict` | `blocked` | Three deterministic repeats produce one verdict fingerprint. |
| `per-case-completeness` | `blocked` | Raw evidence includes every pass, failure, error, and skipped row. |
