...
def build_structured_memory_report(root: Path = PROJECT_ROOT) -> dict[str, Any]:
    ...
    if row is None:
        ...
        summary = {
            ...
            "cross_modal_space_id": row.get("cross_modal_space_id"),
            "cross_modal_space_production_eligible": row.get(
            ...
            "precomputed_vector_space_id": row.get("precomputed_vector_space_id"),
            "same_dimension_space_mismatch_rejected": row.get(
...
def render_structured_memory_markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Shared-space mismatch rejected: `{summary.get('same_dimension_space_mismatch_rejected')}`.",
            ...
            *[_check_row(check) for check in checks],
            ...
            f"| Cross-modal routing | `{raw.get('cross_modal_queries', 0)}` typed queries in `{raw.get('cross_modal_space_id')}`, persisted vector rate `{_fmt(raw.get('cross_modal_vectors_persisted_rate'))}`, provenance `{_fmt(raw.get('cross_modal_provenance_rate'))}`. |",
            f"| External vectors | `{raw.get('precomputed_vector_queries', 0)}` strict precomputed-vector queries in `{raw.get('precomputed_vector_space_id')}` over `{', '.join(raw.get('precomputed_vector_target_modalities', []))}`; equal-dimension mismatch rejected `{raw.get('same_dimension_space_mismatch_rejected')}`. |",
...
def _checks(row: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        _check(
            "same_dimension_space_mismatch_rejected",
            row.get("same_dimension_space_mismatch_rejected"),
            True,
            "is",
        ),
