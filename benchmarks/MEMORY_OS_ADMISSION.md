...
| Query audit traffic enables prewarm and predictive workers | `pass` | hot_query_count=1, enabled=['adaptive-forgetting', 'architecture-advice', 'cache-prewarm', 'consolidation', 'maintenance', 'memory-os', 'predictive-prefetch'] | Enable audited query traffic in staging, replay representative traffic, then rerun memory-os-admission. |
...
| Direct adaptive A/B proves Memory OS quality uplift within latency limits | `pass` | schema=wavemind.memory_os_quality_gate.v2, status=pass, task_uplift=0.125, p95_delta_ms=-0.4281999863451347, p95_ratio=-0.08768121613057815 | Run memory_os_ab_benchmark.py and memory_os_quality_gate.py on this release; non-regression and static-retrieval comparisons do not satisfy this requirement. |
| Fresh six-hour remote soak proves 500-cycle lease, retry, and state safety | `pass` | schema=wavemind.memory_os_remote_worker_soak.v1, status=pass, environment=remote_worker_cluster, duration=21600.119, cycles=500, fresh=True, commit_matches=True, checks_pass=True | Run the six-hour remote soak against two or more HTTPS workers and their TLS Redis, then attach the fresh artifact from the exact release commit. |

## Next Actions

- Schedule Memory OS workers with the emitted singleton, lock, and Redis requirements.
