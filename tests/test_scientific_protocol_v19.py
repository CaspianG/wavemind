...
def test_v19_protocol_binds_failed_and_positive_evidence_and_fresh_subjects():
    payload = _load("scientific_memory_protocol_v19.json")
    ...
    for key in (
        "immutable_v17_evidence",
        "failed_v18_opened_diagnostic",
        "opened_diagnostic_evidence",
    ):
        record = payload[key]
    ...
    assert payload["failed_v18_opened_diagnostic"]["ci_lower"] <= 0.0
    assert payload["opened_diagnostic_evidence"]["ci_lower"] > 0.0
    assert payload["opened_diagnostic_evidence"]["official_gate_decision"] is False

    v17 = _load("scientific_memory_protocol_v17.json")
    old_gate = dict(v17["frozen_development_gate"])
    ...
    assert payload["immutable_v17_evidence"]["mab_units_were_opened"] is False
...
def test_v19_keeps_gate_and_freezes_targeted_dual_coverage():
    payload = _load("scientific_memory_protocol_v19.json")
    ...
    assert parameters["memops_complete_dialogue_registration"] is True
    assert parameters["memops_trajectory_sequence_coverage"] is True
    assert parameters["memops_trajectory_operation_only_sequence_coverage"] is True
    assert parameters["memops_update_sequence_coverage"] is False
