...
def test_v20_freeze_binds_every_executable_file_at_exact_sha():
    payload = json.loads(
        (ROOT / "benchmarks" / "scientific_v20_candidate_freeze.json").read_text(
            encoding="utf-8"
        )
    )
    assert validate_artifact_integrity(payload) == []
    assert payload["fresh_v20_development_runs_executed"] == 0
    ...
    protocol = json.loads(
        subprocess.check_output(
            ["git", "show", f"{sha}:benchmarks/scientific_memory_protocol_v20.json"],
            cwd=ROOT,
        ).decode("utf-8")
    )
