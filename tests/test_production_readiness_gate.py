...
def test_production_readiness_gate_reports_current_blockers():
    ...
    assert "upgraded API True" in criteria["operator_autoscaling_repair"]["evidence"]
    assert "network drill pass" in criteria["operator_autoscaling_repair"]["evidence"]
    assert "docker-pause-kind-worker" in criteria["operator_autoscaling_repair"]["evidence"]
    assert "service nodes 4" in criteria["operator_autoscaling_repair"]["evidence"]
    assert "zones 3" in criteria["operator_autoscaling_repair"]["evidence"]
    assert "outage recall 1.0" in criteria["operator_autoscaling_repair"]["evidence"]
    assert "recovery recall 1.0" in criteria["operator_autoscaling_repair"]["evidence"]
    ...
    assert "Kubernetes regions 3" in criteria["active_active_field_crdt"]["evidence"]
    assert "zones 3" in criteria["active_active_field_crdt"]["evidence"]
    assert "failure docker-pause-kind-worker" in criteria["active_active_field_crdt"]["evidence"]
    assert "outage unavailable ['region-b']" in criteria["active_active_field_crdt"]["evidence"]
    assert "outage writes 32" in criteria["active_active_field_crdt"]["evidence"]
    assert "outage convergence 1.0" in criteria["active_active_field_crdt"]["evidence"]
